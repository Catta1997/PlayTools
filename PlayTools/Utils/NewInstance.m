...
+ (void)load {
    if(@available(iOS 13.2, *)) {
        [objc_getClass("UIApplicationDelegate") swizzleInstanceMethod:@selector(application:shouldRestoreSecureApplicationState:) withMethod:@selector(pm_return_false)];
    }
    else {
        [objc_getClass("UIApplicationDelegate") swizzleInstanceMethod:@selector(application:shouldRestoreApplicationState:) withMethod:@selector(pm_return_false)];
    }
}
@end
